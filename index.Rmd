...
When you have downloaded all files we can unpak them using the `untar` method. Make sure you set the `inputPath` to the actual location of the data you wish the unpack.
```{r, eval=FALSE, results='hide'}
...
To limit the processing time of the `BfastSpatial` methods we will first crop all Landsat NDMI and CFMask scenes to the extent of our area of interest: the extent of the forestMask. So let's load our forest mask and the Landsat scenes
