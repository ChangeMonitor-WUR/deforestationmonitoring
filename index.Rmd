...
Now you can submit your order. You will receive an email when your order is ready for download. Use a download manager (e.g. Bulk Download Application or DownloadThemAll plugin for Firefox) to download all .tar.gz files and save them in a new subfolder in the `data` folder and name it `path5_row68`. 

When you have downloaded all files we can unpak them using the `untar` function. Make sure you set the `inputPath` to the actual location of the data you wish the unpack.
```{r, eval=FALSE, results='hide'}
#set the path location to the data
inputPath <- "data/path5_row68"

#load Landsat tar files and untar these in the data folder 
files <- list.files(inputPath, full.names = TRUE)

for (file in files) {
  untar(file, exdir = inputPath)
}
``` 

Now we are ready to start pre-processing the scenes for further analysis

##2. Pre-processing the Peru scenes

To limit the processing time of the 'BfastSpatial' functions we will first crop all Landsat NDMI and CFMask scenes to the extent of our area of interest: the extent of the forestMask. So let's load our forest mask and the Landsat scenes

```{r, eval=FALSE, results='hide'}
#Load forest mask and list Landsat scenes
forestMask <- raster("data/forestmask/NFM_2001_2231.tif")
rasterList <- list.files(inputPath, pattern = "*.tif", full.names = TRUE)
...
```{r, eval=FALSE, results='hide'}
#crop all scenes to the extent of the mask and save to disk
for (i in 1:length(rasterList)) {
  r <- raster(rasterList[i])
  cropped <- extend(crop(r, forestMask), forestMask)
  
  outfile <- sprintf("%s/%s_cropped", inputPath, names(cropped))
  
  writeRaster(cropped, filename = outfile, datatype = "INT2S", overwrite=TRUE, progress ="text")
}
